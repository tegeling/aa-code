({
  onAfterScriptsLoaded: function (c, e, h) {
    ...
    const p_getFields = fieldNames.length
    ...
    const p_getDefaultValues = recordId
      ? h.getDefaultValues(c, h, recordId, defaultValues.join(","))
          ...
          fields.forEach(function (field, index) {
            if (!field) {
              ...
              h.showError(
                c,
                h,
                `The type '${field.name}' of '${field.objectName}' is not a queryable field.`
              );
              fields[index] = null;
            } else if (
              field.type == "ADDRESS" ||
              ...
              h.showError(
                c,
                h,
                `The type '${field.type}' for '${field.name}' of '${field.objectName}' is unsupported.`
              );
              fields[index] = null;
            } else {
              field.index = ++index;
        ...
        $A.getCallback(function ([fields, defaultValues]) {
          ...
          fields.forEach(function (field) {
            if (!field) {
              ...
              const max = defaultValues.shift();
              field.minValue = min
                ? moment(min).format("YYYY-MM-DDTHH:mm:ss.SSSZ")
                : "";
              field.maxValue = max
                ? moment(max).format("YYYY-MM-DDTHH:mm:ss.SSSZ")
                : "";
            } else if (field.type === "TIME") {
              const min = defaultValues.shift();
              const max = defaultValues.shift();
              field.minValue = min
                ? moment.utc("1970-01-01T" + min).format("HH:mm:ss")
                : "";
              field.maxValue = max
                ? moment.utc("1970-01-01T" + max).format("HH:mm:ss")
                : "";
      ...
      .then(
        $A.getCallback(function ([fields, defaultValues]) {
          ...
          if (!customLogic) {
            ...
            customLogic = "0";
            customLogic +=
              indices.length > 0 ? " AND " + indices.join(" AND ") : "";
          }
  ...
  onSearch: function (c, e, h) {
    ...
    fields.forEach(function (field) {
      if (field.type === "DATETIME") {
        ...
        if (field.maxValue)
          field.maxValue = moment(field.maxValue).format(
            "YYYY-MM-DDThh:mm:ssZ"
          );
      } else if (field.type === "TIME") {
        if (field.minValue && !field.minValue.endsWith("Z"))
          field.minValue += "Z";
        if (field.maxValue && !field.maxValue.endsWith("Z"))
          field.maxValue += "Z";
      } else if (field.type === "BOOLEAN") {
